<div class="sidebar ">
    ...
    <div class="list-group list-group-flush">
        <a href=" {{ route('store.merchant.dashboard',auth()->user()->id) }}"
            class="list-group-item button-rounded {{ Request::is('dashboard') ? 'active' : '' }}  waves-effect mt-3">
            <i class="fa fa-pie-chart mr-3"></i>Dashboard
        </a>
        <a href="{{ route('store.products',auth()->user()->id) }}"
        ...
        <a href="{{ route('store.category',auth()->user()->id) }}"
        ...
        <a href="{{ route('store.merchant.orders',auth()->user()->id) }}"
        ...
        <a href="{{ route('store.users',auth()->user()->id) }}"
